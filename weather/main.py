...
def read_csv_file(file_path: str):
    ...
    df['temperature'] = df['temperature'].apply(float)
    df['precipitation'] = df['precipitation'].apply(float)
    df['rain'] = df['rain'].apply(float)
    df['showers'] = df['showers'].apply(float)
    df['snowfall'] = df['snowfall'].apply(float)
    df['wind_speed_10m'] = df['wind_speed_10m'].apply(float)
    df['wind_speed_80m'] = df['wind_speed_80m'].apply(float)
    df['wind_speed_120m'] = df['wind_speed_120m'].apply(float)
    df['wind_speed_180m'] = df['wind_speed_180m'].apply(float)
    df['temperature_80m'] = df['temperature_80m'].apply(float)
    df['temperature_120m'] = df['temperature_120m'].apply(float)
    df['temperature_180m'] = df['temperature_180m'].apply(float)
    df['direct_radiation'] = df['direct_radiation'].apply(float)
    print(df)
